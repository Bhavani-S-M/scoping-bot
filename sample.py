...
def generate_xlsx(scope: Dict[str, Any]) -> io.BytesIO:
    try:
        ...
        ws_a.set_column("B:C", 30)    # Activities + Description
        ws_a.set_column("D:E", 20)    # Owner + Resources
        ws_a.set_column("F:H", 12)    # Dates
        ws_a.set_column("I:J", 15)    # Effort, DurationTemp
        ...
        for r, a in enumerate(data.get("activities", []), start=2):
            ...
            try:
                s = datetime.fromisoformat(a.get("Start Date"))
            ...
            try:
                e = datetime.fromisoformat(a.get("End Date"))
        ...
        if starts and ends:
            ...
            gantt.add_series({
                ...
                "values": f"='Activities'!$J$2:$J${last_a}",      # DurationTemp col
                "fill": {"color": "#4D96FF"},
                "border": {"color": "#4D96FF"}
            })
