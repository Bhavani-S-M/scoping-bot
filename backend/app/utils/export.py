...
def _safe_str(v, d=""): return d if v is None else str(v)
...
def normalize_scope(scope: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for a in data.get("activities") or []:
        try:
            if a.get("Start Date"):
                start_dates.append(datetime.fromisoformat(a["Start Date"]))
            if a.get("End Date"):
                end_dates.append(datetime.fromisoformat(a["End Date"]))
        except:
            pass
    ...
    for idx, a in enumerate(data.get("activities") or [], start=1):
        try:
            ...
            s = min_start
        try:
            e = datetime.fromisoformat(a.get("End Date", "")) if a.get("End Date") else s
        except:
            e = s
        ...
        # Collect roles: Owner + Depends on (unique set)
...
def generate_xlsx(scope: Dict[str, Any]) -> io.BytesIO:
    try:
        ...
        for i, (k, v) in enumerate(data["overview"].items(), start=2):
        ...
        starts, ends = [], []
        for r, a in enumerate(data["activities"], start=2):
            zfmt = fmt_z1 if r % 2 else fmt_z2
            ws_a.write(r-1, 0, a["ID"], zfmt)
            ws_a.write(r-1, 1, a["Story"], zfmt)
            ws_a.write(r-1, 2, a["Activities"], zfmt)
            ws_a.write(r-1, 3, a["Description"], zfmt)
            ws_a.write(r-1, 4, a["Owner"], zfmt)
            ws_a.write(r-1, 5, a["Depends on"], zfmt)
            try:
                s = datetime.fromisoformat(a["Start Date"])
                ws_a.write_datetime(r-1, 6, s, fmt_date)  # keep date format
                starts.append(s)
            except:
        ...
        last_a = len(data["activities"]) + 1

        # Add structured table with formulas
        ws_a.add_table(
            f"A1:J{last_a}",
            {
        ...
        # Add formulas for efforts and cost columns
        ...
        for c in range(len(res_headers)):
            if c == 0:
                ws_r.write(last_r, c, "Total", fmt_total)   # label in first cell
            else:
...
def generate_pdf(scope: Dict[str, Any]) -> io.BytesIO:
    ...
    #---overview table ----
    ov_rows = [["Field", "Value"]] + [[k, str(v)] for k, v in data["overview"].items()]
    ...
    t = Table(rows, repeatRows=1, colWidths=[25, 90, 90, 120, 70, 100, 60, 60, 40])
    ts = TableStyle([
        ("GRID", (0, 0), (-1, -1), 0.5, colors.black),
        ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor(THEME["header_bg"])),
        ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
    ...
    if parsed:
        ...
        for bi, batch in enumerate(batches, start=1):
            ...
            while cur <= max_e:
                ...
                d.add(String(x+2, 10, cur.strftime("%b %Y"), fontSize=6, fillColor=colors.grey))
            ...
            if bi < len(batches): elems.append(PageBreak())

    # ---- Resourcing Plan table ----
    ...
    for r in plan:
        ...
        eff = float(r.get("Efforts", 0)); cost = float(r.get("cost", eff * r.get("Rate/month", 2000)))
        if rk not in merged:
            merged[rk] = {"Resources": r["Resources"], "Efforts": eff,
                          "Rate/month": r["Rate/month"], "cost": cost,
                          "months": [float(r.get(m,0)) for m in mkeys]}
        else:
            m = merged[rk]
            m["Efforts"] += eff; m["cost"] += cost
            m["months"] = [x+y for x,y in zip(m["months"], [float(r.get(m,0)) for m in mkeys])]
    ...
    rows.append(["Total","",""]+[""]*len(mkeys)+[tot_eff, f"${tot_cost:,.2f}"])
    t2 = Table(rows, repeatRows=1, colWidths=[30,120,70]+[60]*len(mkeys)+[50,65])
    ts2 = TableStyle([
        ("GRID",(0,0),(-1,-1),0.5,colors.black),
        ("BACKGROUND",(0,0),(-1,0),colors.HexColor(THEME["header_bg"])),
        ("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),
        ("BACKGROUND",(0,len(rows)-1),(-1,len(rows)-1),colors.HexColor(THEME["total_bg"]))
    ])
    ...
    if pie_labels:
        ...
        pie.data = pie_vals; pie.labels = pie_labels
        pal = THEME["palette"]
